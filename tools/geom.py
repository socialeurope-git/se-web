...
A,B=sys.argv[1],sys.argv[2]; ROOT=sys.argv[3] if len(sys.argv)>3 else '#main'; DEPTH=int(sys.argv[4]) if len(sys.argv)>4 else 4; W=int(sys.argv[5]) if len(sys.argv)>5 else 1440
JS="""([root,depth])=>{const out=[]; const walk=(el,d,path)=>{ if(d>depth) return; for(const c of el.children){ if(['SCRIPT','STYLE','LINK'].includes(c.tagName)) continue; const r=c.getBoundingClientRect(); const cs=getComputedStyle(c); const id=c.tagName.toLowerCase()+(c.id?'#'+c.id:'')+(c.className&&typeof c.className==='string'?'.'+c.className.split(' ').filter(Boolean).slice(0,2).join('.'):''); out.push({p:path+'>'+id, r:[Math.round(r.x),Math.round(r.y+scrollY),Math.round(r.width),Math.round(r.height)], d:cs.display, fs:cs.fontSize, lh:cs.lineHeight, m:cs.marginTop+'/'+cs.marginBottom, pd:cs.paddingTop+'/'+cs.paddingBottom}); walk(c,d+1,path+'>'+id);} }; const r=document.querySelector(root); if(!r) return []; walk(r,1,''); return out;}"""
async def main():
    ...
    for x,y in zip(a,bb):
        if x['p']!=y['p']: print('STRUCTURE differs at',x['p'],'|',y['p']); break
        if x['r']!=y['r'] or x['d']!=y['d']:
